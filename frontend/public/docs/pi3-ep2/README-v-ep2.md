...
**Disciplina:** Projeto Integrado 3 - Entregável Parcial 2 (EP2)  
...
O sistema **TrAcEs (Trilha de Acompanhamento Escolar)** é uma solução Web de gestão acadêmica focada na transparência e no acompanhamento pedagógico inclusivo. Ele preenche a lacuna existente na comunicação entre instituições de ensino e os lares, permitindo o monitoramento de desempenho e assiduidade escolar em tempo real.

### Problema que o Sistema Resolve

Muitas escolas de pequeno porte, ONGs comunitárias e cursos livres sofrem com a dispersão de informações acadêmicas e a dependência de processos manuais ou planilhas isoladas. Isso gera falhas de cálculo de médias, perda de histórico e atraso na detecção de problemas de rendimento ou de evasão escolar. Além disso, as plataformas de gestão existentes costumam apresentar interfaces complexas e de baixa acessibilidade, gerando exclusão digital de familiares com menor familiaridade tecnológica ou limitações físicas.

### Objetivo do MVP

O MVP do TrAcEs visa validar a viabilidade técnica e a aceitação prática de uma interface web focada na acessibilidade prática (padrões WCAG 2.1 AA) e no design simplificado (Heurísticas de Nielsen). O sistema permite que professores realizem lançamentos rápidos em lote (de notas e presença) e que responsáveis consigam visualizar com clareza a rotina e o status de aprovação de seus dependentes imediatos.

### Usuários do Sistema

- **Responsáveis Legais (Pais/Mães/Tutores):** Usuários de consumo que demandam altíssima acessibilidade e clareza visual para monitorar a frequência anual e notas dos filhos.
- **Corpo Docente (Professores):** Usuários operacionais de escrita que precisam de ferramentas ágeis e seguras para entrada de dados em lote com validação em tempo real.
- **Secretaria e Administradores:** Responsáveis por cadastrar, matricular alunos e configurar os vínculos socioafetivos entre estudantes e responsáveis.

### Organização Geral da Aplicação

A aplicação adota um modelo **Cliente-Servidor desacoplado**:

- **Frontend (Apresentação):** Construído em React + Vite + Shadcn UI, funcionando como uma Single Page Application (SPA) que executa diretamente no navegador do usuário e faz requisições assíncronas de dados.
- **Backend (Regras e Persistência):** Construído em Python seguindo os preceitos da *Clean Architecture* (Domain, Application, Infrastructure), que encapsula as regras de negócio de domínio (validações, cálculo de médias bimestrais e consolidação de boletins) e realiza operações de persistência no banco relacional local SQLite.
- **Comunicação:** Realizada via chamadas HTTP REST sem estado utilizando cargas de dados estruturadas em JSON.

---

## 🏗️ O que é Arquitetura de Software?

A **Arquitetura de Software** representa a estrutura organizacional e de fundação de um sistema computacional. Ela define as diretrizes de alto nível, os componentes de software fundamentais, suas responsabilidades específicas, padrões de interação e os contratos de comunicação que unem essas partes. Em termos simples, a arquitetura de software funciona como o projeto estrutural e mapa de engenharia que orienta o desenvolvimento, a evolução e a sustentabilidade de uma aplicação ao longo do tempo.

### Importância no Desenvolvimento de Sistemas

A arquitetura estabelece as bases técnicas para assegurar que um software seja robusto o suficiente para funcionar corretamente diante de falhas e flexível o bastante para mudar com o tempo. Sem um projeto arquitetural sólido, o código evolui de forma desordenada (código espaguete), elevando drasticamente a incidência de falhas e tornando qualquer alteração futura financeiramente inviável. A arquitetura viabiliza a conversão de requisitos não funcionais (atributos de qualidade como segurança, velocidade e manutenibilidade) em decisões técnicas concretas.

### Impacto nas Áreas Técnicas

- **Escalabilidade:** A arquitetura dita a capacidade de um sistema suportar um aumento contínuo de usuários, requisições ou dados sem degradação do serviço. Por meio do desacoplamento Cliente-Servidor e do *Repository Pattern* (no backend Python), o TrAcEs pode expandir sua capacidade de armazenamento no futuro simplesmente trocando o driver SQLite por um banco de dados hospedado em nuvem (ex: PostgreSQL) sem afetar a lógica acadêmica.
- **Segurança:** A arquitetura define as zonas de confiança, barreiras de isolamento e fluxos autorizados de dados. No TrAcEs, o controle de acesso é arquitetado na autenticação para diferenciar as rotas de escrita do docente das rotas de consulta do responsável, além de assegurar que as validações ocorram na camada de Domínio, agindo como guardiãs da integridade dos dados e prevenindo falhas de injeção ou corrupção de dados físicos.
- **Desempenho:** A organização estrutural gerencia como os recursos de hardware são alocados. Ao adotar uma arquitetura baseada em SPA no frontend, o processamento visual de renderização ocorre no navegador do usuário. O backend atua de forma otimizada devolvendo apenas dados compactos em JSON, gerando transições instantâneas e economizando banda e processamento de rede.
- **Manutenção:** A divisão em camadas isoladas (Clean Architecture) garante o Princípio de Responsabilidade Única (SRP). Se um bug for reportado no cálculo de faltas do boletim, a correção ocorre estritamente dentro da classe de serviço apropriada na camada de Aplicação, sem risco de corromper o banco de dados ou a exibição visual das tabelas.
- **Evolução do Sistema:** Uma boa arquitetura minimiza o acoplamento de código. Isso permite plugar novas integrações (ex: inteligência artificial para detecção precoce de evasão escolar, ou integrações de notificação SMTP por email) sem a necessidade de reescrever as rotas de API existentes ou redesenhar as tabelas de notas de alunos.

### Influência na Qualidade Geral do Projeto

A arquitetura de software é a principal responsável pela qualidade de um projeto de software. Ela assegura que o sistema permaneça compreensível para novos programadores, viabiliza a execução de testes automatizados unitários e de integração confiáveis e garante uma experiência do usuário (UX) estável e sem interrupções. Um projeto com boa arquitetura garante maior ciclo de vida, diminuição drástica no custo de manutenção (TCO) e entrega um produto de software resiliente, performático e seguro para a comunidade de cuidado e para a instituição escolar.
...
Mantimemo-nos em uma estrutura organizada para facilitar a navegação:

```text
sistema_acompanhamento_escolar_pi3-ep2/
...
│   │   └── 📁 diagramas_arquitetura/   # Diagramas técnicos de arquitetura (PNG)
...
│   └── 📁 pi3-ep2/                    # Artefatos da entrega PI3-EP2 (Modelo Arquitetural)
│       ├── 📋 EP2_INDICE.md           # Índice do EP2 (Atual)
│       ├── 🗺️ EP2_ARQUITETURA.md      # Modelo Arquitetural Detalhado
│       └── 📖 README.md               # Cópia da documentação PI3-EP2
...
│       └── 📄 schema.sql              # Schema de tabelas/índices do banco
...
ETAPA 3: Prototipagem UI/UX ✨ (CONCLUÍDO) - Entregável Parcial 1
...
ETAPA 4: Modelo Arquitetural do MVP Web ✨ (CONCLUÍDO) - Entregável Parcial 2 (26/06/2026)
├─ Especificação de arquitetura de software (EP2_ARQUITETURA.md)
├─ Diagramas de arquitetura, componentes, camadas, fluxo e sequência (Mermaid + PNG)
├─ Manual de engenharia e modelagem das rotas de API em JSON
├─ Recuperação e correção da persistência do banco de dados (schema.sql)
└─ Autoavaliação e conformidade técnica
```
...
**Versão:** 4.0 (Entregável Parcial 2 - Modelo Arquitetural)  
**Data da Última Atualização:** 26/06/2026  
**Status:** ✅ Pronto para próxima etapa (Desenvolvimento do Frontend Conectado)
